...
function normalizeScore(value) {
  ...
  // Backend is intended to return 0-100.
  // This also safely handles an older 0-1 response.
...
function AIAnalyzer() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      try {
        ...
        const loadedFarms = Array.isArray(farmsData) ? farmsData : [];
        ...
        if (loadedAssets.length > 0) {
          setSelectedAssetId(String(loadedAssets[0].asset_id));
        }
      } catch (err) {
        console.error("Unable to load AI Analyzer data:", err);
  ...
  const selectedAsset = useMemo(() => {
    return assets.find(
      (asset) => String(asset.asset_id) === String(selectedAssetId)
    );
  ...
  const selectedFarm = useMemo(() => {
    ...
    return farms.find(
      (farm) => Number(farm.farm_id) === Number(selectedAsset.farm_id)
    );
  }, [farms, selectedAsset]);
  ...
  const healthScore = normalizeScore(analysis?.health_score);
  const riskScore = normalizeScore(analysis?.risk_score);
      ...
      <section className="mt-8 rounded-[28px] border border-[#E4E9E1] bg-white p-7 shadow-[0_12px_40px_rgba(32,39,34,0.04)] sm:p-9">
        ...
        {loadingAssets ? (
          <div className="mt-8 flex items-center gap-3 rounded-2xl bg-[#F7F9F6] p-5 text-sm text-[#738078]">
            <Loader2 size={18} className="animate-spin text-[#001e61]" />
            Loading your assets...
          </div>
        ) : assets.length === 0 ? (
            ...
            <div className="mt-8 grid gap-5 lg:grid-cols-[1fr_auto] lg:items-end">
              <div>
                ...
                <p className="mt-3 text-xs text-[#89968E]">
                  {selectedFarm?.farm_name || "Renewable Energy Farm"}
              ...
              <button
                ...
                disabled={analyzing || !selectedAsset}
            ...
            <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <h2 className="mt-2 text-2xl font-semibold tracking-[-0.035em] text-[#202722]">
                  {selectedAsset?.asset_label || `Asset ${analysis.asset_id}`}
                </h2>
          ...
          <div className="grid gap-6 lg:grid-cols-[1fr_1fr]">
            <div className="rounded-[28px] border border-[#E4E9E1] bg-white p-7 shadow-[0_12px_40px_rgba(32,39,34,0.04)] sm:p-8">
              ...
              <div className="mt-7 rounded-2xl bg-[#FFF9EA] p-5">
                <p className="text-sm font-semibold text-[#202722]">
                  {analysis.probable_issue || "No specific issue detected"}
                </p>
              ...
              <div className="mt-6">
                ...
                <div className="mt-3 space-y-2">
                  {analysis.contributing_factors ? (
                    String(analysis.contributing_factors)
                      .split(";")
                      .map((factor, index) => (
                        <div
                          key={`${factor}-${index}`}
          ...
          <div className="rounded-[28px] border border-[#E4E9E1] bg-white p-7 shadow-[0_12px_40px_rgba(32,39,34,0.04)] sm:p-8">
            <div className="flex items-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#F0F3F9] text-[#001e61]">
                {selectedAsset?.asset_type === "solar" ? (
              ...
              <div>
                ...
                <h3 className="mt-1 text-lg font-semibold text-[#202722]">
                  {selectedAsset?.asset_label || "Selected Asset"}
                </h3>
              </div>
            </div>
