...
class Farm(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "energy_type IN ('solar', 'wind', 'mixed')",
...
class AIAnalysis(Base):
    ...
    # Energy loss is estimated in kWh/day.
    # Revenue loss is estimated in INR/day.
    energy_loss_estimate = Column(Float, nullable=True)
    revenue_loss_estimate = Column(Float, nullable=True)

    __table_args__ = (
        CheckConstraint(
            "status IN ('healthy', 'watch', 'at_risk')",
            name="chk_status",
        ),
    )
...
class MaintenanceJob(Base):
    ...
    analysis_id = Column(
        Integer,
        ForeignKey("ai_analyses.analysis_id", ondelete="CASCADE"),
        nullable=False,
    )
